...
import {
  ACTIVITIES, CONDITIONS, EMPTY_DRAFT, MEDICATIONS, OLDER_ADULT_AGE, OUTDOOR_OPTIONS,
  type OutdoorOption, type PersonaDraft,
  addPersona, markOnboarded, modelledRisks, selectedPersona, updatePersona,
} from "@/lib/persona";
...
function ProfilePage() {
  ...
  // Arriving from "Edit profile" loads the person currently being scored, and
  // saving changes THEM rather than creating a second one. Arriving fresh from
  // login starts blank.
  const [editing] = useState(() => selectedPersona());
  const [draft, setDraft] = useState<PersonaDraft>(() => {
    const current = selectedPersona();
    return current ? { ...EMPTY_DRAFT, ...current } : EMPTY_DRAFT;
  });
  ...
  const toggle = (key: "conditionIds" | "medicationIds") => (id: string) =>
    setDraft((previous) => ({
      ...previous,
  ...
  const toggleCondition = toggle("conditionIds");
  const toggleMedication = toggle("medicationIds");

  // What the model will actually act on, shown live so there is no surprise
  // between what you tick and what moves the score.
  const risks = useMemo(
    () => modelledRisks(draft.conditionIds, Number(draft.age) || 0, draft.medicationIds),
    [draft.conditionIds, draft.medicationIds, draft.age],
  );
    ...
    <div className="min-h-dvh bg-canvas text-foreground">
      ...
      <main className="mx-auto max-w-[900px] px-5 py-8 md:px-8 md:py-12">
        ...
        <h1 className="mt-2 text-3xl font-bold md:text-4xl">
          {editing ? `Editing ${editing.name || "this profile"}` : "Who are we planning for?"}
        </h1>
        ...
        <form
          ...
          onSubmit={(event) => {
            ...
            if (editing) {
              updatePersona(editing.uid, draft);
              markOnboarded();
              // Straight back to the scores: the point of editing is to watch
              // them move, and a detour through the week view hides that.
              navigate({ to: "/" });
            } else {
              addPersona(draft);
              markOnboarded();
        ...
        >
              ...
              {CONDITIONS.map((condition) => (
                ...
                >
                  ...
                  <span className="min-w-0">
                    {condition.label}
                    {condition.maps_to ? (
                      <span className="ml-2 whitespace-nowrap rounded-full bg-primary/10 px-2 py-0.5 text-[0.7rem] font-semibold text-primary">
                        affects score
                      </span>
                    ) : (
                      <span className="ml-2 whitespace-nowrap rounded-full bg-muted px-2 py-0.5 text-[0.7rem] font-semibold text-muted-foreground">
                        not yet modelled
                      </span>
                    )}
                  </span>
          ...
          <section className="rounded-xl border border-border bg-background p-6 shadow-panel">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Some medications change how a body handles heat — antihistamines suppress
              sweating, beta blockers cut blood flow to the skin, diuretics reduce blood
              volume. This is sent to our own scoring endpoint and is not stored anywhere.
            </p>
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
            ...
            <p className="mt-3 text-xs leading-relaxed text-muted-foreground">
              Ticking more than one does not stack — the model has a single medication
              factor today, so any one of these moves the score by the same amount.
            </p>
          ...
          <div className="flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
            <Button
              ...
              onClick={() => { markOnboarded(); navigate({ to: editing ? "/" : "/week" }); }}
            >
              {editing ? "Discard changes" : "Skip for now"}
            </Button>
            <Button type="submit" className="h-11 rounded-xl px-6 text-base font-semibold">
              <Check /> {editing ? "Save and rescore" : "Save and continue"}
            </Button>
          </div>
        </form>
      </main>
