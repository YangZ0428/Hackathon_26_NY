...
export const Route = createFileRoute("/profile")({
  head: () => ({
    meta: [
      { title: "Your profile — Climap NYC" },
      {
        name: "description",
        content:
          "Tell Climap NYC a little about you so recommendations fit your routine, health considerations, and location.",
      },
      { property: "og:title", content: "Your profile — Climap NYC" },
      { property: "og:description", content: "Add your details so Climap NYC can personalise activity suggestions." },
...
// Static mock list — replace with real medication data when wiring up the app.
const medicationTypes = [
  ...
  "Corticosteroids (e.g., inhalers, prednisone)",
];
...
function ProfilePage() {
  ...
  const [otherMedChecked, setOtherMedChecked] = useState(false);
  ...
  return (
    <div className="min-h-dvh bg-canvas text-foreground">
      ...
      <main className="mx-auto max-w-[900px] px-5 py-8 md:px-8 md:py-12">
        <p className="text-sm font-bold uppercase text-primary">Welcome to Climap NYC</p>
        <h1 className="mt-2 text-3xl font-bold md:text-4xl">Tell me a little about you</h1>
        <p className="mt-2 max-w-xl text-base leading-relaxed text-muted-foreground">
          This helps me suggest activities that fit your routine and how you feel outside. You can change any of it
          later.
        </p>
        ...
        >
          <section className="rounded-xl border border-border bg-background p-6 shadow-panel">
            ...
            <div className="mt-5 grid gap-5 sm:grid-cols-2">
              ...
              <div className="space-y-1.5 sm:col-span-2">
                ...
                <Input id="location" defaultValue="New York, NY" className="h-11" />
              </div>
            </div>
          </section>
          ...
          <section className="rounded-xl border border-border bg-background p-6 shadow-panel">
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Some medications can affect how heat, sun, or air quality feel for you — this stays private and is only
              used to keep suggestions comfortable.
            </p>
            <div className="mt-5 grid gap-3 sm:grid-cols-2">
              {medicationTypes.map((medication) => (
                ...
                >
                  <Checkbox
                    ...
                    onCheckedChange={() => toggle(medications, setMedications, medication)}
                  />
                  {medication}
                </label>
              ))}
              <label className="flex cursor-pointer items-center gap-3 rounded-lg border border-border px-3.5 py-2.5 text-sm font-medium transition-colors has-[[data-state=checked]]:border-primary has-[[data-state=checked]]:bg-primary/5">
                <Checkbox
                  checked={otherMedChecked}
                  onCheckedChange={(checked) => setOtherMedChecked(checked === true)}
                />
                Other
              </label>
            </div>
            {otherMedChecked && (
              <div className="mt-4 space-y-1.5">
                <Label htmlFor="other-medication">Please describe</Label>
                <Input id="other-medication" placeholder="Any medications I should keep in mind?" className="h-11" />
              </div>
            )}
          </section>
