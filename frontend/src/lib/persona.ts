...
export const CONDITIONS: ConditionOption[] = [
  ...
  { id: "heat_medication", label: "A medication that affects heat tolerance",
...
export const EMPTY_DRAFT: PersonaDraft = {
  name: "", age: "", conditionIds: [], outdoor: "Regular",
...
export function modelledRisks(conditionIds: string[], age: number): RiskCategory[] {
...
export function draftToProfile(draft: PersonaDraft): Profile {
  ...
  const risks = modelledRisks(draft.conditionIds, age);
  return {
    id: "custom",
...
export const loadPersonas = (): StoredPersona[] => read<StoredPersona[]>(KEY, []);
